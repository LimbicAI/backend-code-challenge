...
describe('PostResolver (e2e)', () => {
  ...
  const posts: string[] = JSON.parse(
    fs.readFileSync(Path.join(__dirname, "../test/fixtures/posts.json"), "utf8")
  )
  ...
  beforeAll(async () => {
    ...
    existingUser = await request(app.getHttpServer())
      ...
      .send({
        query:
          `mutation{ createUser(createUserData: {email: "${testUser.email}", password: "${testUser.password}"}){ id email } }`,
  ...
  describe('should add new posts', () => {
    ...
    it.each(posts)("create post (%s)", async (post: any) => {
      return await request(app.getHttpServer())
        ...
        .send({
          query:
            `mutation{ createPost(createPostInput: { text: "${post.text}"}){ text, user{ id } } }`,
        })
        ...
        .expect((res) => {
          ...
          expect(data.createPost.user && data.createPost.user.id).toBe(existingUser.id);
        });
    });
  })
  ...
  describe('should not add new add posts - invalid token', () => {
    ...
    it.each(posts)("create post (%s)", async (post: any) => {
      return await request(app.getHttpServer())
        ...
        .set('authorization', `Bearer eyJlbWFpbCI6InRlc3RAdGVzdC5jb20iLCJzdWIiOjM0NSwiaWF0IjoxNjU5NDU0NzQ5LCJleHAiOjE2NTk0NTgzNDl9.XI6EYvDzXZ2fdUyFBMLTR3CgwDnBRQF4lhNWXGiMGrg`)
        .expect(200)
        .expect((res) => {
